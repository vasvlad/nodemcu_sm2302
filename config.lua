...
config.DOMAIN = "domain"                        -- The area of scope, like "home" or "cottage"
config.LOCATION = "location" 					-- The LOCATION of the nodeMCU sensor in the domain, keep the name short and avoid spaces, ie. "garage" or "mainbedroom"
config.EVENTTYPE = "temphumi"					-- The event type.  Using "temphumi" for this sensor.
config.ledRed = 0                               -- gpio16

-- WiFi Configuration
config.targetSSID = "YourWiFiSSID" 				-- The name that your wifi is broadcasting
config.targetSSIDPassword = "YourWifiPassword" 	-- Replace with your wifi password
config.connectWait = 3000 						-- How long to wait for the wifi connection to establish before checking the status again

-- MQTT Configuration
config.HOST = "YourTargetMQTT" 					-- IP address or hostname of your MQTT server 
config.PORT = 1883 								-- Standard MQTT port, don't change unless you know why you are changing it
config.USERNAME = "username"					-- MQTT Username
config.PASSWD = "password"						-- MQTT Password
...
config.PUBINTERVAL = 60000						-- How often the sensor should publish its readings, in milliseconds. Examples: 1000 = 1 sec, 60000 = 1 min, 360000 = 6 min (10 data points an hour)
